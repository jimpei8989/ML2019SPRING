...
def ReadTrainingData(path):
    ...
    one_X = np.concatenate([data[480*m + h : 480*m + h+9, 9 : 10].reshape((1, -1)) for h in range(471) for m in range(12)], axis = 0)
    two_X = np.concatenate([np.concatenate([(x.reshape((-1, 1)) * x.reshape((1, -1)))[i:i+1, :i+1].reshape((1, -1)) for i in range(one_X.shape[1])], axis = 1).reshape((1, -1)) for x in one_X], axis = 0)
    Y = np.concatenate([data[480*m + h+9, 9].reshape((1, -1)) for h in range(471) for m in range(12)], axis = 0)
    return np.concatenate([np.ones((one_X.shape[0], 1)), one_X, two_X], axis = 1), Y

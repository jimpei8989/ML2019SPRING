...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.Decoder = nn.Sequential(
                nn.ConvTranspose2d(96, 48, kernel_size = 5, stride = 2, padding = 2, output_padding = 1),
                nn.ReLU(),
                nn.ConvTranspose2d(48, 24, kernel_size = 5, stride = 2, padding = 2, output_padding = 1),
                nn.ReLU(),
                nn.ConvTranspose2d(24,  3, kernel_size = 5, stride = 2, padding = 2, output_padding = 1),
                nn.ReLU(),
                )
...
def main():
    ...
    print('--- Begin K-Means ---')
    kmeans = KMeans(n_clusters = 2, n_jobs = 16, random_state = luckyNumber)

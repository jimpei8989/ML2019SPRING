...
def ReadTrainingData(path):
    ...
    X = np.concatenate([np.array([float(f) for f in x.split()]).reshape((1, -1)) for x in df['feature']], axis = 0)
    Y = np.zeros((num, Ydim))
...
if __name__ == "__main__":
    ...
    model.add(Dense(4096, kernel_regularizer = l2(1e-6)))
    model.add(BatchNormalization())
    model.add(ReLU())
    model.add(Dropout(0.5))

    model.add(Dense(2048, kernel_regularizer = l2(1e-6)))
    model.add(BatchNormalization())
    model.add(ReLU())
    model.add(Dropout(0.5))
    ...
    model.add(Dense(1024, kernel_regularizer = l2(1e-6)))
    model.add(BatchNormalization())
    model.add(ReLU())
    model.add(Dropout(0.5))
    ...
    history = model.fit(X, Y, batch_size = 128, epochs = 400, validation_split = 0.1)

...
def ReadTrainingData(path, std = False):
    ...
    X = np.concatenate([zdata[480*m + h : 480*m + h+5, 9].reshape((1, -1)) for h in range(475) for m in range(12)], axis = 0)
    Y = np.concatenate([rdata[480*m + h+5, 9].reshape((1, -1))             for h in range(475) for m in range(12)], axis = 0)
